...
class Settings:
    ...
    def __init__(self):
        ...
        self.markdown_header_template = """# Jerome's Daily Log, {}
...
if __name__ == "__main__":
    ...
    if directory_exists(today_dir_path):
        ...
        with open(os.path.join(today_dir_path, today_dir_path + ".md"), "w") as f:
            f.write(settings.markdown_header_template.format(settings.cute_date_format(current_datetime)))
    ...
    for elem in ls():
        if directory_exists(elem):
            today_dir_path = elem
            cd(today_dir_path)
            shell_cmd("pandoc {} -o {}".format(today_dir_path + ".md", today_dir_path + ".pdf"))
            cd(calling_dir)
    print("Done.")
